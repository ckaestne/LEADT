...
public static enum MESSAGE_TYPE { WARNING, ERROR, INFO, ELEMENT, NONE }
	...
	public void createPartControl(Composite parent) {
		...
		viewer.setContentProvider(contentProvider = new NonFeatureContentProvider(this));
	...
	private void makeActions() {
		...
		deleteElementAction = new Action() {
			public void run() {
				...
				Map<AIElement, IFeature> elementsToRemove = new HashMap<AIElement, IFeature>();
				...
				for (TreePath treePath : paths) {
					...
					elementsToRemove.put((AIElement)node.getDataObject(),NonFeatureTreeNode.getColor(node));
				}
				
				if (elementsToRemove.size()>0)
					AC.fireEvent(new AElementsNonColorChangedEvent(this, new HashMap<AIElement, IFeature>(), elementsToRemove));
	
			}
		};
		deleteElementAction.setText("Remove");
		deleteElementAction.setToolTipText("Mark as - possible recommendation for features.");
		deleteElementAction.setImageDescriptor(PlatformUI.getWorkbench().getSharedImages().
			getImageDescriptor(ISharedImages.IMG_TOOL_DELETE));
		deleteElementAction.setDisabledImageDescriptor(PlatformUI.getWorkbench().getSharedImages().
		...
		selectionChangedAction = new Action() {
			public void run() {
				...
				try {
					...
					AC.fireEvent(new AElementPreviewEvent(NonFeatureManagerView.this));
					
					
					IEditorPart javaEditor;
					javaEditor = JavaUI.openInEditor(AC.getICompilationUnit(cuHash));
