...
class NonFeatureContentProvider implements IStructuredContentProvider,
	...
	//	keys2ASTDummys = new HashMap<String, ASTDummy>();
		...
		invisibleRoot = new NonFeatureTreeNode(NonFeatureTreeNode.NODE_KIND.ROOT,"",0);
		isInit = false;
		checkIsIntialized();	
	...
	private boolean checkIsIntialized() {
		...
		if (project == null) {
			nonFeatureManager.setInfoMessage("Data base has not been created for Feature Mining", MESSAGE_TYPE.ERROR);
			return false;
		}
		
		nonFeatureManager.setInfoMessage("Data base been created for " + project.getName(), MESSAGE_TYPE.INFO);
		isInit = true;
	...
	private int[] calculateCounts(NonFeatureTreeNode curNode) {
		...
		if (curNode.getKind() == NODE_KIND.ELEMENT) {
			return new int[] {curNode.getViewCount(), curNode.getElementsCount()};
		}
	...
	private NonFeatureTreeNode getColorNode(IFeature color) {
		...
		NonFeatureTreeNode tmpFeatureNode = new NonFeatureTreeNode(NonFeatureTreeNode.NODE_KIND.FEATURE, color, addCount);
	...
	private NonFeatureTreeNode getCompilationUnitNode(NonFeatureTreeNode featureNode, int cuHash) {
		...
		NonFeatureTreeNode tmpCUNode = new NonFeatureTreeNode(NonFeatureTreeNode.NODE_KIND.COMPILATION_UNIT, dummy, addCount);
	...
	private NonFeatureTreeNode getElementNode(NonFeatureTreeNode featureNode, AIElement element) {
	...
	private void addElement(IFeature color, AIElement element) {
		
		NonFeatureTreeNode tmpFeatureNode = getColorNode(color);
		NonFeatureTreeNode tmpCUNode = getCompilationUnitNode(tmpFeatureNode, element.getCompelationUnitHash());
		
		NonFeatureTreeNode tmpElementNode = new NonFeatureTreeNode(NonFeatureTreeNode.NODE_KIND.ELEMENT, element, addCount);
		tmpCUNode.addChild(tmpElementNode);
			

		//SELECT THE NEW AST NODE
		//build the path
		selections.add( new TreePath(new Object[] {tmpFeatureNode, tmpCUNode, tmpElementNode}));	
		
	}
	
	private void removeElement(IFeature color, AIElement element) {
...
//			}
	...
	public void elementNonColorsChanged(AElementsPostNonColorChangedEvent event) {	
		...
		for (AIElement element : event.getAddedElements().keySet()) {
			IFeature color = event.getAddedElements().get(element);
			addElement(color, element);
		}
		
		for (AIElement element : event.getRemovedElements().keySet()) {
			...
			for (NonFeatureTreeNode cu : feature.getChildren()) {
				for (NonFeatureTreeNode element : cu.getChildren()) {
					if (((AIElement)element.getDataObject()).equals(elementToSelect)) {
						selections.add(new TreePath(new Object[] {feature, cu, element}));
					}
				}
	...
	private void setSelections() {
		...
		if (selections.size() > 0) {
			...
			nonFeatureManager.getTreeViewer().setSelection(new TreeSelection(selArray),true);
		}
	...
	public void update(Observable o, Object arg) {
		if (o.equals(AC)) {
			...
			if (arg instanceof AInitEvent) {
				nonFeatureManager.setInfoMessage("Data base been created for " + ((AInitEvent)arg).getProject().getName(), MESSAGE_TYPE.INFO);
				isInit = true;
