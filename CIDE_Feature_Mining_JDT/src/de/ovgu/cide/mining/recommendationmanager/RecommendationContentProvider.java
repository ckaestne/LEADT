...
class RecommendationContentProvider implements IStructuredContentProvider,
	...
	public RecommendationContentProvider(
			RecommendationManagerView featureManager) {
		this.recommendationManager = featureManager;
		this.AC = ApplicationController.getInstance();
		invisibleRoot = new RecommendationTreeNode(
				RecommendationTreeNode.NODE_KIND.ROOT, null, null, null);
	...
	private boolean checkIsIntialized() {
		...
		if (project == null) {
			recommendationManager.setInfoMessage(
	...
	public void showRecommendations(ARecommenderElementSelectedEvent event) {
		...
		if (event.getType().equals(EVENT_TYPE.ELEMENT)) {
			...
			recommendations = AC.getRecommendations(event.getColor(),
					sourceElement);
		} else {
			recommendations = AC.getRecommendations(event.getColor(),
					event.getStart(), event.getEnd(), event.getCuHash());
		}

		for (AElement tmpElement : recommendations.keySet()) {
			ARecommendationContextCollection collection = recommendations
					.get(tmpElement);
			RecommendationTreeNode tmpCollectionNode = new RecommendationTreeNode(
					NODE_KIND.CONTEXTCOLLECTION, tmpElement, event.getColor(),
					collection);
			invisibleRoot.addChild(tmpCollectionNode);
			for (ARecommendationContext context : collection.getContexts()) {
				tmpCollectionNode.addChild(new RecommendationTreeNode(
						NODE_KIND.CONTEXT, context.getSupporter(), null,
						context));
			}
		}
		...
		if (invisibleRoot.getChildrenCount() > 0) {
			recommendationManager.setInfoMessage(
					"Recommendations for selected element", MESSAGE_TYPE.INFO);
		} else {
			recommendationManager.setInfoMessage(
					"There are no recommendations for the selected element.",
					MESSAGE_TYPE.WARNING);
	...
	public void update(Observable o, Object arg) {
		if (o.equals(AC)) {
			...
			if (arg instanceof AInitEvent) {
				recommendationManager.setInfoMessage("Database created for "
