...
public class RelationManagerView extends ViewPart  {
	...
	private LinkedList<AIElement> historyList;
	...
	public void createPartControl(Composite parent) {
		...
		viewer.setContentProvider(contentProvider = new RelationContentProvider(this));
		...
		historyList = new LinkedList<AIElement>();
	...
	public void addToHistory (AIElement element) {
		if (element == null)
			return;
	...
	private void showHistory() {
		...
		for (AIElement tmpElement : historyList) {
	...
	private void makeActions() {
		...
		doubleClickAction = new Action() {
			public void run() {
				...
				AIElement jayElement = (AIElement)((RelationTreeNode)obj).getDataObject();
		...
		selectionChangedAction = new Action() {
			public void run() {
				...
				try {
					...
					javaEditor = JavaUI.openInEditor(jayFX.getICompilationUnit(jayElement.getCompelationUnitHash()));
					
					if (javaEditor instanceof ITextEditor) {
						
						jayFX.fireEvent(new AElementPreviewEvent(RelationManagerView.this));
						//UnifiedASTNode node = jayElement.getUnifiedASTNode();
						((ITextEditor)javaEditor).selectAndReveal(jayElement.getStartPosition(), jayElement.getLength());
						//((ITextEditor)javaEditor).etHighlightRange(node.getStartPosition(), node.getLength(),true);
