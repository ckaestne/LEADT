...
public class AAnalyzer 
{
	 ...
	 * @pre pDB != null
	 */
	public AAnalyzer( ProgramDatabase pDB )
	{
		assert( pDB != null );
		aDB = pDB;
	}
...
//			(pRelation == Relation.HAS_RETURN_TYPE) || (pRelation == Relation.T_HAS_RETURN_TYPE) ||
//			
...
//	}
	...
	{
		Set<AIElement> lReturn = new HashSet<AIElement>();
		if( (pRelation == ARelation.DECLARES_FIELD) || (pRelation == ARelation.T_DECLARES_FIELD) ||
			(pRelation == ARelation.DECLARES_IMPORT ) || (pRelation == ARelation.T_DECLARES_IMPORT ) || 
			(pRelation == ARelation.DECLARES_LOCAL_VARIABLE ) || (pRelation == ARelation.T_DECLARES_LOCAL_VARIABLE ) ||
			(pRelation == ARelation.DECLARES_METHOD) || (pRelation == ARelation.T_DECLARES_METHOD ) ||
			(pRelation == ARelation.DECLARES_TYPE_TRANSITIVE) || (pRelation == ARelation.T_DECLARES_TYPE_TRANSITIVE ) ||
			(pRelation == ARelation.DECLARES_METHOD_TRANSITIVE) || (pRelation == ARelation.T_DECLARES_METHOD_TRANSITIVE ) ||
			(pRelation == ARelation.DECLARES_FIELD_TRANSITIVE) || (pRelation == ARelation.T_DECLARES_FIELD_TRANSITIVE ) ||
			(pRelation == ARelation.DECLARES_LOCAL_VARIABLE_TRANSITIVE) || (pRelation == ARelation.T_DECLARES_LOCAL_VARIABLE_TRANSITIVE ) ||
			(pRelation == ARelation.IMPLEMENTS_TYPE) || (pRelation == ARelation.T_IMPLEMENTS_TYPE ) ||
			(pRelation == ARelation.EXTENDS_TYPE) || (pRelation == ARelation.T_EXTENDS_TYPE ) ||
			(pRelation == ARelation.IMPLEMENTS_TYPE_TRANSITIVE) || (pRelation == ARelation.T_IMPLEMENTS_TYPE_TRANSITIVE) ||
			(pRelation == ARelation.EXTENDS_TYPE_TRANSITIVE) || (pRelation == ARelation.T_EXTENDS_TYPE_TRANSITIVE ) ||
			(pRelation == ARelation.OVERRIDES_METHOD) || (pRelation == ARelation.T_OVERRIDES_METHOD ) ||
			...
			(pRelation == ARelation.ACCESS_METHOD) || (pRelation == ARelation.T_ACCESS_METHOD ) ||
			(pRelation == ARelation.ACCESS_TYPE_TRANSITIVE) || (pRelation == ARelation.T_ACCESS_TYPE_TRANSITIVE ) ||
			(pRelation == ARelation.ACCESS_FIELD_TRANSITIVE) || (pRelation == ARelation.T_ACCESS_FIELD_TRANSITIVE ) ||
			(pRelation == ARelation.ACCESS_LOCAL_VARIABLE_TRANSITIVE) || (pRelation == ARelation.T_ACCESS_LOCAL_VARIABLE_TRANSITIVE ) ||
			(pRelation == ARelation.ACCESS_METHOD_TRANSITIVE) || (pRelation == ARelation.T_ACCESS_METHOD_TRANSITIVE ) ||
			(pRelation == ARelation.BELONGS_TO) || (pRelation == ARelation.T_BELONGS_TO) ||
			(pRelation == ARelation.REQUIRES) || (pRelation == ARelation.T_REQUIRES) ||
			(pRelation == ARelation.DECLARES_FIELD_ACCESS) || (pRelation == ARelation.T_DECLARES_FIELD_ACCESS) ||
			(pRelation == ARelation.DECLARES_LOCAL_VARIABLE_ACCESS) || (pRelation == ARelation.T_DECLARES_LOCAL_VARIABLE_ACCESS) ||
			(pRelation == ARelation.DECLARES_TYPE_ACCESS) || (pRelation == ARelation.T_DECLARES_TYPE_ACCESS) ||
			(pRelation == ARelation.DECLARES_METHOD_ACCESS) || (pRelation == ARelation.T_DECLARES_METHOD_ACCESS) ||
			(pRelation == ARelation.DECLARES_TYPE) || (pRelation == ARelation.T_DECLARES_TYPE )){
			lReturn.addAll( aDB.getRange( pElement, pRelation ));
		}
		else if( pRelation == ARelation.DECLARES ) {
			lReturn = getDeclares( pElement );
		}
		else if( pRelation == ARelation.T_DECLARES ) {
		    lReturn = getT_Declares( pElement );
		}
		else if( pRelation == ARelation.ACCESSES ) {
		    lReturn = getAccess( pElement );
		}
		else if( pRelation == ARelation.T_ACCESS ) {
		    lReturn = getT_Access( pElement );
		}
		else if( pRelation == ARelation.REFERENCES ) {
		    lReturn = getReferences( pElement );
		}
		else if( pRelation == ARelation.T_REFERENCES ) {
	...
	public Set<AIElement> getDeclares( AIElement pElement) {
		Set<AIElement> declaresElements = new HashSet<AIElement>();
		declaresElements.addAll( aDB.getRange( pElement, ARelation.DECLARES_FIELD ));
		declaresElements.addAll( aDB.getRange( pElement, ARelation.DECLARES_IMPORT ));
		declaresElements.addAll( aDB.getRange( pElement, ARelation.DECLARES_LOCAL_VARIABLE ));
		declaresElements.addAll( aDB.getRange( pElement, ARelation.DECLARES_METHOD ));
		declaresElements.addAll( aDB.getRange( pElement, ARelation.DECLARES_TYPE ));
		
		declaresElements.addAll( aDB.getRange( pElement, ARelation.DECLARES_FIELD_ACCESS ));
	...
	public Set<AIElement> getT_Declares( AIElement pElement) {
		Set<AIElement> declaresElements = new HashSet<AIElement>();
		declaresElements.addAll( aDB.getRange( pElement, ARelation.T_DECLARES_FIELD ));
		declaresElements.addAll( aDB.getRange( pElement, ARelation.T_DECLARES_IMPORT ));
		declaresElements.addAll( aDB.getRange( pElement, ARelation.T_DECLARES_LOCAL_VARIABLE ));
		declaresElements.addAll( aDB.getRange( pElement, ARelation.T_DECLARES_METHOD ));
		declaresElements.addAll( aDB.getRange( pElement, ARelation.T_DECLARES_TYPE ));
		
		declaresElements.addAll( aDB.getRange( pElement, ARelation.T_DECLARES_FIELD_ACCESS ));
	...
	public Set<AIElement> getReferences( AIElement pElement) {
		Set<AIElement> accessElements = new HashSet<AIElement>();
		accessElements.addAll( aDB.getRange( pElement, ARelation.ACCESS_FIELD ));
		accessElements.addAll( aDB.getRange( pElement, ARelation.ACCESS_LOCAL_VARIABLE ));
		accessElements.addAll( aDB.getRange( pElement, ARelation.ACCESS_METHOD ));
		accessElements.addAll( aDB.getRange( pElement, ARelation.ACCESS_TYPE ));
		
		Set<AIElement> referenceElements = new HashSet<AIElement>();
	...
	public Set<AIElement> getT_References( AIElement pElement) {
		Set<AIElement> accessElements = new HashSet<AIElement>();
		accessElements.addAll( aDB.getRange( pElement, ARelation.T_BELONGS_TO ));
		
		Set<AIElement> referenceElements = new HashSet<AIElement>();
		
		for (AIElement accessElement : accessElements) {
			referenceElements.addAll( aDB.getRange( accessElement, ARelation.T_ACCESS_FIELD ));
			referenceElements.addAll( aDB.getRange( accessElement, ARelation.T_ACCESS_LOCAL_VARIABLE ));
			referenceElements.addAll( aDB.getRange( accessElement, ARelation.T_ACCESS_METHOD ));
			referenceElements.addAll( aDB.getRange( accessElement, ARelation.T_ACCESS_TYPE ));
		}
...
//		assert(pElement instanceof ATypeElement);
//		
//		Set<AIElement> lToProcess = aDB.getRange( pElement, Relation.T_EXTENDS_CLASS );
//	    Set<AIElement> lReturn = new HashSet<AIElement>();
//	    
...
//		Set<AIElement> lSuperclass = aDB.getRange( pElement, Relation.EXTENDS_CLASS );
//		
