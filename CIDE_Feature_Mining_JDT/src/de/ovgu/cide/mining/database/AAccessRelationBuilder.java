...
//						aDB.addRelationAndTranspose(curType, ARelation.EXTENDS_TYPE, typeAccessElement);
//							
//					}
//					else if (descriptor == TypeDeclaration.SUPER_INTERFACE_TYPES_PROPERTY ||
//							descriptor == EnumDeclaration.SUPER_INTERFACE_TYPES_PROPERTY) {
//						aDB.addRelationAndTranspose(curType, ARelation.IMPLEMENTS_TYPE, typeAccessElement);
//					} 
//					else {
//						aDB.addRelationAndTranspose(curType, ARelation.ACCESS_TYPE, typeAccessElement);
//					}
//					
			...
			private boolean collectExtendsAndImplementsTypeRelations(ITypeBinding declTypeBinding,
				...
				if (declTypeBinding == null || transitiveTypes == null || directTypes == null)
					return false;			
				...
				for (ITypeBinding tmpInterface : interfaces) {
					...
					OverridingRelationUtils.collectExtendedAndImplementedTypesInInterfaces(tmpInterface,
							transitiveTypes, checkedInterfaces);
				}
				...
				if (superClass != null) {
					...
					OverridingRelationUtils.collectExtendedAndImplementedTypesInSuperClasses(
							superClass, transitiveTypes, checkedInterfaces);
				}
			...
			private void createInherritedAndOverriddenMethodRelations(IMethodBinding binding ) {
					...
					for (MethodPathItem methodPathItem : inhMethods) {
						...
						if (superMethod != null) {
							...
							if (!methodPathItem.isAbstract()) {
								aDB.addRelationAndTranspose(curMethod, overridesRelation, superMethod);
							}
							else {
								aDB.addRelationAndTranspose(curMethod, implementsRelation, superMethod);
							}
						}
			...
			private boolean collectInherritedOrOverridenMethods(IMethodBinding binding,
			...
			public void endVisitFieldOrVariable(ASTNode node, IVariableBinding binding) {
			...
			private void addContextForFieldorVariable(ASTNode contextNode, SimpleName nameNode) {
				...
				if (binding != null && binding instanceof IVariableBinding) {
					...
					curContext = new LocalContextElement(contextNode, nameNode, curElement);		
			...
			public void visitName(Name node) {
				...
				if (binding instanceof ITypeBinding) {
					...
					handleFieldOrVariableAccess(node,(IVariableBinding) binding);
				}
...
//				}
			...
			private void handleTypeAccessForVarDeclFragment(ASTNode node){
				...
				case ASTNode.VARIABLE_DECLARATION_STATEMENT:
					type = ((VariableDeclarationStatement)parentNode).getType();
					break;
				case ASTNode.VARIABLE_DECLARATION_EXPRESSION:
					type = ((VariableDeclarationExpression)parentNode).getType();
					break;
				case ASTNode.FIELD_DECLARATION:
			...
			private void handleFieldOrVariableAccess(Name node, IVariableBinding binding) {
				...
				aDB.addRelationAndTranspose(curCUElement, accessTransitiveRelation, accessElement);
				
				//ADD ALWAYS TRANS. ACCESS RELATION FOR TYPE
				if (curType != null) {
				
					aDB.addRelationAndTranspose(curType, accessTransitiveRelation, accessElement);
...
//TODO: CHECK SUPER ACCESS FOR OUT OF CONTEXT ELEMENTS!
				...
				if (curContext != null) {
					if (curContext.getElement() == null){
						...
						aDB.addRelationAndTranspose(curContext.getElement(), accessRelation, accessElement);
						directRelationAdded = true;
					}
				}
				...
				for (int i = 0; i < size; i++) {
					...
					if (!directRelationAdded && size - i == 1) {
						...
						if (tmpContext.getElement() == null)
							...
							aDB.addRelationAndTranspose(tmpContext.getElement(), accessRelation, accessElement);		
						
						continue;
						
					}
					if (tmpContext.getElement() != null)
						aDB.addRelationAndTranspose(tmpContext.getElement(), accessTransitiveRelation, accessElement);				
					
				}		
		
				if (curLocalVariable != null) {
					if (directRelationAdded) {
						aDB.addRelationAndTranspose(curLocalVariable, accessTransitiveRelation, accessElement);
					}
					else {
						aDB.addRelationAndTranspose(curLocalVariable, accessRelation, accessElement);
						directRelationAdded = true;		
					}
				}
				...
				if (curField != null) {
					if (directRelationAdded) {
						aDB.addRelationAndTranspose(curField, accessTransitiveRelation, accessElement);	
					}
					else {
						aDB.addRelationAndTranspose(curField, accessRelation, accessElement);	
						directRelationAdded = true;
					}
				}
				...
				if (curMethod != null) {
						aDB.addRelationAndTranspose(curMethod, declaresRelation, accessElement);
					
					if (directRelationAdded) {
						aDB.addRelationAndTranspose(curMethod, accessTransitiveRelation, accessElement);
					}
					else {
						aDB.addRelationAndTranspose(curMethod, accessRelation, accessElement);
					}
				}
