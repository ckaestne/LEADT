...
public class AElementViewCountManager implements ISelectionListener, Observer {
	...
	private Map<AIElement, Integer> element2Views;
	...
	public AElementViewCountManager(ApplicationController AC) {
		this.AC = AC;
		oldSelection = null;
		element2Views = new HashMap<AIElement, Integer> ();
		previewSource = null;
		
		AC.addObserver(this);
		ISelectionService service = CIDECorePlugin.getDefault().getWorkbench().getActiveWorkbenchWindow().getSelectionService();		
	...
	public void selectionChanged(IWorkbenchPart part, ISelection selection) {
		if (selection instanceof ITextSelection) {
			...
			for(AIElement element : AC.getAllElements()) {
				...
				AC.fireEvent(new AElementViewCountChangedEvent(this, element,  previewSource));
