...
public class TypeCheckElementRecommender extends AAbstractElementRecommender {
	...
	public TypeCheckElementRecommender() {
		...
		try {
			model = FeatureModelManager.getInstance().getFeatureModel(
					AC.getInitializedProject());
	...
	public Map<AElement, ARecommendationContext> getRecommendations(
		...
		Set<AElement> accessElements = AC.getRange(element,
				ARelationKind.T_BELONGS_TO);
		for (AElement accessElement : accessElements) {
			...
			if (!check.evaluate(strategy)
					&& isValidRecommendation(accessElement, color)) {
				// access element should be recommended
				ARecommendationContext context = new ARecommendationContext(
						element, "Check Accesses", getRecommendationType(), 1);
		...
		Set<AElement> paramTargetElements = AC.getRange(element,
				ARelationKind.REQUIRES);
		for (AElement paramTargetElement : paramTargetElements) {

			Object[] bodyTargetElements = AC.getRange(paramTargetElement,
					ARelationKind.T_DECLARES_PARAMETER).toArray();
			AElement bodyTargetElement = null;
			if (bodyTargetElements.length > 0)
				bodyTargetElement = (AElement) bodyTargetElements[0];

			Set<AElement> bodySourceElements = AC.getRange(bodyTargetElement,
			...
			InvocationCheck check = new InvocationCheck(element,
					paramTargetElement, bodySourceElement, bodyTargetElement,
					model);
			...
			if (!check.evaluate(strategy)) {
				...
				if (paramTargetElement != null
						&& isValidRecommendation(paramTargetElement, color)) {
					solutionsCount++;
					solutions[1] = true;
				}
		...
		if (element.getCategory().equals(AICategories.PARAMETER_ACCESS)) {
			...
			Object[] paramSourceElements = AC.getRange(paramTargetElement,
					ARelationKind.T_REQUIRES).toArray();
			...
			InvocationCheck check = new InvocationCheck(paramSourceElement,
					paramTargetElement, bodySourceElement, bodyTargetElement,
					model);
			...
			if (!check.evaluate(strategy)) {
				...
				if (paramSourceElement != null
						&& isValidRecommendation(paramSourceElement, color)) {
					solutionsCount++;
					solutions[1] = true;
				}
